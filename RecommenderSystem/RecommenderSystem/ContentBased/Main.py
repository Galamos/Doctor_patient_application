...
class Main:
    ...
    ratingsPath = '../Data/use_case1.csv'
    specialistOriginal = '../Data/specialistLens.csv'
    ...
    def getAttributes(self):
        ...
        #convert integer-encoded attribute list to bitfields that can be treated as vectors
    ...
    def getSpecialty(self):
        ...
        specialtiesIDs = {}
        maxSpecialties = 0
        with open(self.specialistOriginal, newline='', encoding='ISO-8859-1') as csvfile:
            specialistReader= csv.reader(csvfile)
            #skip header line
            next(specialistReader)
            
            for row in specialistReader:
                specialistID = float(row[0])
                specialtiesList = row[2]
                specialtiesIDList = []
                
                for specialty in specialtiesList:
                    if specialty in specialtiesIDs:
                        specialtyID = specialtiesIDs[specialty]
                    else:
                        specialtyID = maxSpecialties
                        specialtiesIDs[specialty] = specialtyID
                        maxSpecialties += 1
                    specialtiesIDList.append(specialistID)
                specialties[specialistID] = specialtiesIDList
